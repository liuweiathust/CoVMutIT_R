...
DocumentsTab <- tabItem(
			  tabName = "documents",
			    fluidPage(
				                   mainPanel(
							                    h1(strong("Material and methods")),
									                   h2(strong("The CoVMutIT framework")),
									                   h3(p("The CoVMutIT framework outlines three defining principles:"),
											                        p("1. A more transmissible common mutation increases in frequency deterministically toward fixation, and the trajectory of frequency change can be summarized into several different inductive patterns;"),
														                  p("2. The increasing in mutant frequency is statistically significant;"),
														                  p("3. The observation is reproducible in multiple geographical regions for widespread mutations. "),
											                     ...
											                     h3("The mutations of each SARS-CoV-2 sample have been detected using in-house scripts with the multiple alignment files as input data, then CorGAT20,21 is used for the functional annotation of identified mutations. To track the progress of the mutations accumulation and spreading, the mutant frequency in population is estimated by simply divide the number of viral genome assemblies carried a specific mutation by the total number of assemblies available in given geographical region. The monthly average of mutant frequency is calculated in each geographical region to construct a complete timeline for coronavirus spreading and mutations accumulation. Subsequently, concurrent mutations have been identified with in-house scripts, and the Genome-wide linkage disequilibrium analysis was performed by computing r2 values for all pairwise mutations comparisons in VCFtools22 (version 0.1.16) with haploid mode on. To build the phylogenetic tree, the genome assemblies is subsampled and processed using the nextstrain23 ncov pipeline (https://github.com/nextstrain/ncov), then the phylogenetic tree is generated and visualized with Auspice23."),
													                    ...
													                    h3(p("The coronavirus samples of consecutive two-months from the same geographical region are regarded as two distinct sub-populations, and the changes in mutant frequency will be used to estimate the genetic drift while coronavirus spreading. The Balding-Nichols model24 is employed to estimate the F statistics, which represents average genetic drift between two sub-populations, then a beta-distribution testing is used to estimate the exact genetic drift strength of each mutation site between sub-populations. The Balding-Nichols model adapted in our research could be described with following distribution (formula 1)."),
															                         img(src = "formula_01.png", height = 30, width = 102),
																		                   ...
																		                   img(src = "formula_02.png", height = 30, width = 102),
